...
SELECT
    ...
    round(sum(unit_price * unit_quantity), 0)                                          AS total_gross_value,
    round(sum(CASE WHEN product_category = 'Base'   THEN 1 ELSE 0 END)*100.0/count(*),1) AS pct_base,
    round(sum(CASE WHEN product_category = 'Add-on' THEN 1 ELSE 0 END)*100.0/count(*),1) AS pct_addon,
    round(sum(CASE WHEN product_category = 'Camera' THEN 1 ELSE 0 END)*100.0/count(*),1) AS pct_camera
FROM enriched
GROUP BY channel
